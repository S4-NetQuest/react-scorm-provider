...
describe('SCORM v2004', () => {
  ...
  it('allows retrieval of any valid SCORM data model key with the get method', async () => {
    ...
    let consumer = wrapper.find(ContextConsumer).childAt(0);
    let sco = consumer.prop('sco');
    const v = await sco.get('cmi.learner_id');
  ...
  it('allows valid writable SCORM data model values to be set with the set method', async () => {
    ...
    let consumer = wrapper.find(ContextConsumer).childAt(0);
    let sco = consumer.prop('sco');
    const v = await sco.set('cmi.learner_preference.language', 'en-US');
    expect(v).toEqual(expect.arrayContaining(['cmi.learner_preference.language', 'en-US']));
    wrapper.unmount();
  });
  ...
  it('rejects invalid or read-only SCORM data model keys sent with the set method', async () => {
    ...
    let consumer = wrapper.find(ContextConsumer).childAt(0);
    let sco = consumer.prop('sco');
    const v = await sco.set('cmi.learner_name', 'I should not be able to set this read-only field').catch(e => e);
    expect(v).toEqual("could not set: { cmi.learner_name: I should not be able to set this read-only field }");
    wrapper.unmount();
  });
  ...
  it('allows score information to be sent with the setScore method', async () => {
    ...
    let iS = sco.get('cmi.score.raw');
    expect(iS).toEqual('');
    let iMin = sco.get('cmi.score.min');
    expect(iMin).toEqual('');
    let iMax = sco.get('cmi.score.max');
    expect(iMax).toEqual('');

    // set values
    let vals = await sco.setScore({ value: 80, min: 0, max: 100 });
    expect(vals).toContainEqual(['cmi.score.raw', 80]);
    expect(vals).toContainEqual(['cmi.score.min', 0]);
    expect(vals).toContainEqual(['cmi.score.max', 100]);
    ...
    let s = sco.get('cmi.score.raw');
    expect(Number(s)).toEqual(80);
    let min = sco.get('cmi.score.min');
    expect(Number(min)).toEqual(0);
    let max = sco.get('cmi.score.max');
